...
try:
    ...
    ISOTPSocket("vcan0", sid=0x705, did=0x712, extended_addr=0x41, extended_rx_addr=0x41,
                padding=False, basecls=ISOTP),
    ISOTPSocket("vcan0", sid=0x713, did=0x707, extended_addr=0x42, extended_rx_addr=0x55,
                padding=True, basecls=ISOTP),
    ISOTPSocket("vcan0", sid=0x73c, did=0x777, extended_addr=0x43, extended_rx_addr=0x49,
                padding=False, basecls=ISOTP),
    ISOTPSocket("vcan0", sid=0x789, did=0x755, extended_addr=0x44, extended_rx_addr=0x58,
                padding=True, basecls=ISOTP),
    ISOTPSocket("vcan0", sid=0x7ee, did=0x702, extended_addr=0x45, extended_rx_addr=0x50,
                padding=True, basecls=ISOTP),
    ISOTPSocket("vcan0", sid=0x7ab, did=0x7cd, extended_addr=0x46, extended_rx_addr=0x59,
                padding=False, basecls=ISOTP),]
    print("ISOTP Endpoints initialized.")
    while True:
        time.sleep(100)
    ...
    for count, socket in enumerate(socks):
        socks[count].close()
        print(f"Closed socket: {socks[count]}")
